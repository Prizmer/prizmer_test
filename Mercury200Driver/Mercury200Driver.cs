...
namespace Drivers.Mercury200Driver
{
    public class Mercury200Driver : CMeter, IMeter
    {
        ...
        private int FindPacketSignature(Queue<byte> queue)
        {
            try
            {
                ...
                this.WriteToLog("FindPacketSignature: несовпадение байт в пакете");
                return -1;
            }
            catch (Exception ex)
            {
                this.WriteToLog("FindPacketSignature: исключение " + ex.Message);
                return -2;
            }
        }
